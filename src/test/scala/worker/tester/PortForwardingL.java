...
public class PortForwardingL {
	...
	public static void main(String[] arg) {
		...
		try {
			...
			String config = JOptionPane.showInputDialog("Enter -L port:host:hostport", "port:host:hostport");
			lport = Integer.parseInt(config.substring(0, config.indexOf(':')));
			config = config.substring(config.indexOf(':') + 1);
			rhost = config.substring(0, config.indexOf(':'));
			rport = Integer.parseInt(config.substring(config.indexOf(':') + 1));
